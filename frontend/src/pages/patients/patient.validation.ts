...
const patientValidation = object().shape({
  ...
  height: number().test("decimalDigits", "Altura é obrigatório. Ou valor é inválido.", (number) =>
    /\d+\.?\d*/g.test(String(number))
  ),
  weight: number().test("decimalDigits", "Peso é obrigatório. Ou valor é inválido.", (number) =>
    /\d+\.?\d*/g.test(String(number))
  ),
  genre: mixed<Genre>()
